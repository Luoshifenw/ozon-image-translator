...
function App() {
  ...
  const handleDownloadAll = useCallback(() => {
    translatedImages.filter(img => img.status === 'success').forEach(image => {
  ...
  const statusConfig = {
    ...
    uploading: { text: '正在上传...', color: 'text-amber-400', bg: 'bg-amber-900/30' },
    processing: { text: '正在翻译处理中...', color: 'text-blue-400', bg: 'bg-blue-900/30' },
    completed: { text: '翻译完成！', color: 'text-emerald-400', bg: 'bg-emerald-900/30' },
  ...
  const successCount = translatedImages.filter(img => img.status === 'success').length;
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 bg-slate-800/50 rounded-2xl border border-slate-700/50">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-slate-200 mb-2">翻译结果</h2>
                  <p className="text-slate-400">
                    成功 <span className="text-emerald-400 font-semibold">{successCount}</span> 张 / 
                    失败 <span className="text-red-400 font-semibold">{translatedImages.length - successCount}</span> 张
                  </p>
                </div>
                ...
                >
                  {image.status === 'success' ? (
                    <>
                      ...
                      {/* 信息和下载按钮 */}
                      <div className="p-3">
                        <p className="text-xs text-slate-400 mb-2 truncate" title={image.original_name}>
                    ...
                    <div className="p-4 aspect-square flex flex-col items-center justify-center">
                      ...
                      <p className="text-slate-500 text-xs text-center">{image.error || '未知错误'}</p>
                    </div>
                  )}
                </div>
              ))}
